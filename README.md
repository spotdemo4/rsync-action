...
Set `tls: true` for an `rsync-ssl` daemon. With TLS enabled the action checks for `rsync`, `rsync-ssl`, `openssl`, and `stunnel`; otherwise it checks for `rsync`. Missing tools are restored from the Actions tool cache or downloaded from the matching `*-download-url` input and then added to the tool cache with `@actions/tool-cache`.
